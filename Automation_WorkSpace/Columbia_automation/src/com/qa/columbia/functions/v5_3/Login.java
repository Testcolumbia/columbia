...
public class Login {
	...
	public static void LoginCVP(Selenium sel, String str_Username, String str_Password) throws Exception 
	{
	    ...
	    utilityFunction.waitForElementToVisible(sel, EnvObjectMap_Rep.txt_userNameCVP, 300);
	...
	public static void LoginSVP	(Selenium sel, String str_Username, String str_Password) throws Exception 
	{
        ...
        boolean flag_loginlink=false;
        boolean flag_logoutlink=false;
        
        for (int second = 0; second<=150; second++){
          if (sel.isElementPresent(EnvObjectMap_Rep.lnk_LoginSVP)){
        	   flag_loginlink= true;
        	   sel.click(EnvObjectMap_Rep.lnk_LoginSVP);
        	   break;
          }else { 
        	  flag_logoutlink= true;   
        	  }
	     }
       if (flag_loginlink){
	...
	if(flag_logoutlink){
		System.out.println("EGC user is already logged in");
	}
	}  

...
public class SVP_Sm_vpv_010_VOD_SWF_SL_disabled extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_010_VOD_SWF_SL_disabled() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			String globalVariableFile ="Global_variables.xml";
			String localVariableFile ="SVP_Sm_vpv_010_VOD_SWF_SL_disabled.xml";
			...
			String VAR_SWFfile = utilityFunction.GetValue(localVariableFile ,"VAR_fileType");
			//Variable for SWF file name
			String VAR_SWF_fileName = utilityFunction.GetValue(localVariableFile ,"VAR_SWF_fileName");
			...
			//Reusable action for SWF file upload
     		...
     		Create_Program.create_VODProgram(selenium, str_ProgName,"", VAR_SWF_fileName);
			utilityFunction.waitForChangesToReflect();	
		    ...
		    assertTrue("Sm_vpv_010","Social VP: Ensure that an On Demand program(SWF file) plays with HTTP delivery option if SL is disabled in browser.",date,utilityFunction.assertTextContains(htmlsource, VAR_asxfile));
		    selenium.close();

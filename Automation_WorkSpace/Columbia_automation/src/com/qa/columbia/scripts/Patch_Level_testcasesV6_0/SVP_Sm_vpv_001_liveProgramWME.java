...
/*This  scripts include 2 test cases:1)SVP_Sm_vpv_001_liveProgramWME
 *                                   2)SVP_Sm_vpv_003_ArchiveliveProgramWME
* Class name                                                        SVP_Sm_vpv_001_liveProgramWME 
*
* Version info
*
* Copyright notice
*
* Test Case reference:												Social VP: Ensure that Live program plays with VE as WMS delivery option using WME. (Slide, Polls, Surveys, Questions and archive content)<br>
...
* What Verification it performs:	 								Verify that Live program plays with VE as WMS delivery option using WME. (Slide, Polls, Surveys, Questions and archive content)<br><br> 
*                                                                   Verify the archiving for the Live program
*					
*																															
...
public class SVP_Sm_vpv_001_liveProgramWME extends TesterAsserter {
	...
	public  void test_SVP_Sm_vpv_001_liveProgramWME()throws Exception {
		try{
		setUp();
		Utility_Functions utilityFunction = new Utility_Functions(); 		
		...
		String localVariableFile="SVP_Sm_vpv_001_liveProgramWME.xml";	
		...
		String Var_EncoderName = utilityFunction.GetValue(setupVariableFile, "VAR_WMEName");
		String Var_EncoderProfileName = utilityFunction.GetValue(setupVariableFile, "VAR_WMEProName");
		...
		//Create Live program
		Create_Program.create_LiveProgram(selenium, Var_LiveProgramName, Var_EncoderName, Var_EncoderProfileName, VAR_SlideDisplayName, VAR_PollName, VAR_SurveyName, Var_VEasWMS_DO);
		...
		if(selenium.isElementPresent(EnvObjectMap_Rep.lnk_pushPoll))
		{
			...
			utilityFunction.waitForChangesToReflect();
			utilityFunction.waitForChangesToReflect();
			assertTrue("Sm_vpv_001","Verify that Poll Question is pushed successfully from LBC.",date,selenium.isElementPresent(EnvObjectMap_Rep.lnk_pushPollResult));
		}
		selenium.selectWindow("SVP");
		utilityFunction.waitForElementToVisible(selenium, EnvObjectMap_Rep.LNK_PollWindow, 300);
		
		assertTrue("Sm_vpv_001", "Verify that poll is displayed successfully on the viewer portal.", date, selenium.isElementPresent(EnvObjectMap_Rep.CHK_PollWindow));
	    ...
	    //submit the poll
	    selenium.click(EnvObjectMap_Rep.BTN_PollWindow);
	    assertTrue("Sm_vpv_001", "Poll is submitted successfully", date, true);
	    //wait
		...
		assertTrue("Sm_vpv_001", "Verify that user is able to push the results to the viewer portal", date, true);
		...
		selenium.click(EnvObjectMap_Rep.btn_stopBroadcast);
		 ...
		 for (int second = 0;; second++) {
		    ...
		    assertTrue("Sm_vpv_003","Verify that Archive files are fetched.",date,selenium.isTextPresent("(none)|Archive files fetched.|Completed "));
		...
		assertTrue("Sm_vpv_001","Verify that survey is displayed successfully on viewer portal.",date,selenium.isElementPresent(EnvObjectMap_Rep.btn_SubmitSurveyanswer));
		selenium.click(EnvObjectMap_Rep.btn_SubmitSurveyanswer);
		assertTrue("Sm_vpv_001", "Survey is submitted successfully", date, true);
		...
		utilityFunction.waitForChangesToReflect();
		utilityFunction.waitForChangesToReflect();	
		assertTrue("Sm_vpv_003","Social VP: Ensure that ingest completes for archived content of Live program played with VE as WMS delivery option using WME.",date,selenium.isTextPresent(Var_ArchiveOfLiveprogram));
		utilityFunction=null;

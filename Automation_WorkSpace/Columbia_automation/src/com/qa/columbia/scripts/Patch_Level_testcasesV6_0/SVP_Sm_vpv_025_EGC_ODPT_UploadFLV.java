...
public class SVP_Sm_vpv_025_EGC_ODPT_UploadFLV extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_025_EGC_ODPT_UploadFLV() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			...
			String globalVariableFile ="Global_variables.xml";
			String localVariableFile ="SVP_Sm_vpv_025_EGC_ODPT_UploadFLV.xml";
     	 ...
     	 //click the Domains link in right pane
           ...
           for (int second = 0;; second++) {
  	           ...
  	           try { if (selenium.isElementPresent("link=" +VAR_EGCprogramName))
  	         	  assertTrue("Sm_vpv_025","Social VP: Ensure that Program is successfully created using the FLV file from SVP EGC portal.",date,selenium.isTextPresent(VAR_EGCprogramName));
  	             Launch_VP.Launch_SVP(selenium, VAR_DomSocial);
        		     //selenium.waitForPageToLoad(VAR_MED_WAIT);
           ...
           //call to reusable function for play VOD
		    ...
		    assertTrue("Sm_vpv_025","SVP: Verify that user can create an EGC program with flash file as streaming media, if 'Media Format' as 'Adobe Flash Video' is selected in on-demand program template.",date,utilityFunction.assertTextContains(htmlsource, VAR_asxfile));
			utilityFunction=null;

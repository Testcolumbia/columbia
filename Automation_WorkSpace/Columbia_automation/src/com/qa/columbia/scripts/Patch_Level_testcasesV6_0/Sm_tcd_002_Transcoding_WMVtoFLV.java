/*
* Classname                                                        Transcoding_Sm_tcd_002_WMVtoFLV 
*
* Version info
*
* Copyright notice
...
public class Sm_tcd_002_Transcoding_WMVtoFLV extends TesterAsserter {
	...
	public void test_Sm_tcd_002_Transcoding_WMVtoFLV() throws Exception 
		{
			try{
		setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			String str_path ="Global_variables.xml";
			String local_path ="Sm_tcd_002_TranscodeWMVfiletoflv.xml";
    		...
    		//verify if transcoding node exists otherwise create a new one
     				 ...
     				 for(int sec = 0;; sec++) {
     					 ...
     					 try { 
     						 if (selenium.isElementPresent(EnvObjectMap_Rep.lst_transcodetextfield)) {
     							 ...
     							 utilityFunction.waitForChangesToReflect();
     							 utilityFunction.waitForChangesToReflect();
     							 assertTrue("Sm_tcd_002","Verify that file has transcoded successfully",date,selenium.isTextPresent(VAR_WMV_fileName));
     		...
     		for (int secs = 0;; secs++) {
				 ...
				 try { 
					 ...
					 {
						 assertTrue("Sm_tcd_002","Ensure that Transcoded file is displayed on Content page.",date,selenium.isTextPresent(VAR_WMV_fileName +","+" "+"transcoded"));	
     		 ...
     		 if (str_var.contains(VAR_WMV_fileName +","+" "+"transcoded")){
     			 ...
     			 for (int secs = 0;; secs++) {
     				 ...
     				 try { 
     					 ...
     					 {
     						 assertTrue("Sm_tcd_002","Ensure that ingest is completed for the transcoded file.",date,selenium.isTextPresent("INGESTCOMPLETE"));
     					 }
     					 break; } catch (Exception e) {}
     					 Thread.sleep(1000);
    			 ...
    			 //To verify the asx file
    			 assertTrue("Sm_tcd_002","Verify that asx file is generated successfully for a WMV file transcoded into a flv file.",date,utilityFunction.assertTextContains(htmlsource, VAR_WMV_fileName+","+" "+"transcoded"));
    			 assertTrue("Sm_tcd_002","Verify that wmv file is displayed in the asx file for a WMV file transcoded into a flv file.",date,utilityFunction.assertTextContains(htmlsource, "Transcode.flv"));
    			utilityFunction=null;
			}catch(Exception e)
			{
			e.printStackTrace();

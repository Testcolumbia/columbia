...
* Copyright notice
*
* Test Case reference:												Classic VP: Ensure that a non pin protected On-demand program is created through RSS 2000 Version4 polycom discovery.
...
* 																	>Place a call from the Endpoint to the RSS device and hang call after some time.<br>
...
* What Verification it performs:	 								1.Verify that non Pin-protected On-demand program is created for the Non-pin protected call using RSS2kV4 discovery.<br> 
*																															
...
public class SVP_Sm_vpv_012_VOD_RSSdisc2k_NonPinProtected extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_012_VOD_RSSdisc2k_NonPinProtected() throws Exception {
		{
			try
			{
			setUp();
			...
			String localVariableFile ="SVP_Sm_vpv_012_VOD_RSSdisc2k_NonPinProtected.xml";
			...
			for (int i =4;i<=10;i++)
			{
				...
				selenium.waitForPageToLoad(VAR_MED_WAIT);
				boolean pinStatus;
				if(selenium.getText(EnvObjectMap_Rep.lbl_pinStatusVODSummaryPageSVP).equals("No"))
				{
					pinStatus = true;
					//Verify that non-pin protected program is created for the call
					assertTrue("Sm_vpv_12","Verify that VOD program is created for the archived media.",date,pinStatus);
				}
				break;
				}
			}
			...
			//choose dynamic program
			   outerloop:
				...
				for (int i = 1; i<=12; i++)
				{
					 ...
					 if(str_var1.equals(Var_VODProgramName))
					 {
			...
			//Wait
			 ...
			 String screenShot_SVPvideo = Var_frameWork_Path + "\\commonfiles\\" + Var_VODProgramName + ".jpg";		
		     ...
		     assertTrue("Sm_vpv_12","Verify that http protocol is displayed in asx file.",date,utilityFunction.assertTextContains(htmlsource, "http"));
		     selenium.close();
			 utilityFunction.waitForChangesToReflect();
			 selenium.selectWindow("null");
			 }

...
public class SVP_Sm_vpv_026_EGC_ODPT_UploadWMV extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_026_EGC_ODPT_UploadWMV() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			...
			String globalVariableFile ="Global_variables.xml";
			String localVariableFile ="SVP_Sm_vpv_026_EGC_ODPT_UploadWMV.xml";
		   ...
		   String VAR_userLastName = utilityFunction.GetValue(localVariableFile ,"VAR_userLastName");
			...
			String VAR_streamingMediaEGC = utilityFunction.GetValue(localVariableFile, "VAR_streamingMediaEGC");
			 String VAR_VEasWMSDOPT=utilityFunction.GetValue(globalVariableFile ,"Var_VEasWMS_DO");
     	 ...
     	 //click the Domains link in right pane
     	  ...
     	  Create_ProgramTemplate.test_Create_VODTemplate(selenium, VAR_ProgramTemplateName, VAR_MediaFormat,VAR_VEasWMSDOPT);
           ...
           for (int second = 0;; second++) {
  	           ...
  	           try { if (selenium.isElementPresent("link=" +VAR_EGCprogramName))
  	         	  assertTrue("Sm_vpv_026","Social VP: Ensure that Program is successfully created using the WMV file from SVP EGC portal.",date,selenium.isTextPresent(VAR_EGCprogramName));
  	             Launch_VP.Launch_SVP(selenium, VAR_DomSocial);
        		     //selenium.waitForPageToLoad(VAR_MED_WAIT);
		    ...
		    assertTrue("Sm_vpv_026","SVP: Verify that user can create an EGC program with 'WMV' as streaming media, if 'Media Format' as window media and video edge(as wms) Delivery Option is selected in on-demand program template.",date,utilityFunction.assertTextContains(htmlsource, VAR_asxfile));
			utilityFunction=null;

/*
* Classname                                                        CVP_VOD_RSSdisc4k_NonPinProtected 
*
* Version info
*
* Copyright notice
...
public class CVP_VOD_RSSdisc4k_NonPinProtected extends TesterAsserter {
	...
	public void test_CVP_VOD_RSSdisc4k_NonPinProtected() throws Exception {
		{
			try
			{
			setUp();
		...
		//	String VAR_EndPointIP = utilityFunction.GetValue(local_path ,"VAR_EndPointIP");
			...
			for (int i =4;i<=10;i++)
			{
				...
				if(selenium.getText(EnvObjectMap_Rep.lbl_pinStatusVODSummaryPage).equals("No"))
				{
					...
					assertTrue("CVP_VOD_RSSdisc4k_NonPinProtected","Verify that On-demand program is created for the Non-Pin Protected call.",date,pinStatus);
				}
				break;
				}
			}		
		...
		//Verify the asx file
		assertTrue("CVP_Automated_RSSdisc4k_NonPinProtected","Verify that asx file is generated successfully for the discovered On-demand program.",date,utilityFunction.assertTextContains(htmlsource2, Var_VODProgramName + "hi.asf"));
		assertTrue("CVP_Automated_RSSdisc4k_NonPinProtected","Verify that http protocol is displayed in asx file for the discovered On-demand program.",date,utilityFunction.assertTextContains(htmlsource2, "http"));

...
public class SVP_VOD_FLV_SL_Enabled extends TesterAsserter {
	...
	public void test_SVP_VOD_FLV_SL_Enabled() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			String str_path ="Global_variables.xml";
			String local_path ="SVP_VOD_FLV_SL_Enabled.xml";
			...
			String VAR_FLVfile = utilityFunction.GetValue(local_path ,"VAR_fileType");
			//Variable for FLV file name
			String VAR_FLV_fileName = utilityFunction.GetValue(local_path ,"VAR_FLV_fileName");
			...
			//Reusable action for Login to VCC
     		...
     		Create_Program.test_Create_vodProgram(selenium, str_ProgName, VAR_FLV_fileName);
		...
		//	utilityFunction.assertTextContains(htmlsource, VAR_FLV_fileName);
		//	utilityFunction.assertTextContains(htmlsource, VAR_SiteURL +":80");   
			assertTrue("SVP_VOD_FLV_SL_Enabled","Verify that asx file is generated successfully for a SVP containg .flv file with HTTP delivery option and SL enabled.",date,utilityFunction.assertTextContains(htmlsource, VAR_FLV_fileName));
			assertTrue("SVP_VOD_FLV_SL_Enabled","Verify that http protocol is displayed in asx file for a SVP containg .flv file with HTTP delivery option and SL enabled.",date,utilityFunction.assertTextContains(htmlsource, "http://" + VAR_AP_Host + ":80/" + VAR_AP_MountPoint));
			utilityFunction=null;
		}catch(Exception e){
			e.printStackTrace();}
			 finally{

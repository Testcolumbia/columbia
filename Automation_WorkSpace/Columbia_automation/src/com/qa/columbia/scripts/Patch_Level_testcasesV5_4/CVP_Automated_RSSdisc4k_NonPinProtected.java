...
public class CVP_Automated_RSSdisc4k_NonPinProtected extends TesterAsserter {
	...
	public void test_CVP_Automated_RSSdisc4k_NonPinProtected() throws Exception {
		{
			try
			{
			setUp();
			...
			String local_path ="CVP_Automated_RSSdisc4k_NonPinProtected.xml";
		...
		//	utilityFunction.waitForChangesToReflect();
			...
			selenium.waitForPageToLoad(VAR_MED_WAIT);
			//Click on Polycom RSS devices
			selenium.click(EnvObjectMap_Rep.lnk_PolycomRSSDevices);
			//wait for loading the page
			selenium.waitForPageToLoad(VAR_MED_WAIT);
			//Check if the source device is already added
			...
			for (int i =4;i<=10;i++)
			{
				if(selenium.getText(MessageFormat.format(EnvObjectMap_Rep.lnk_DiscoveredProgram, i)).startsWith(VAR_ProgPrefix))
					...
					assertTrue("CVP_Automated_RSSdisc4k_NonPinProtected","Verify that automated file broadcast program is created for the live call.",date,selenium.isTextPresent(Var_LiveProgramName));
					break;
					}			
			}
			...
			assertTrue("CVP_Automated_RSSdisc4k_NonPinProtected","Verify that asx file is generated successfully for the discovered automated file broadcast program.",date,utilityFunction.assertTextContains(htmlsource2, Var_LiveProgramName + "hi"));
			assertTrue("CVP_Automated_RSSdisc4k_NonPinProtected","Verify that mms protocol is displayed in asx file for the discovered automated file broadcast program.",date,utilityFunction.assertTextContains(htmlsource2, "mms"));
			selenium.close();

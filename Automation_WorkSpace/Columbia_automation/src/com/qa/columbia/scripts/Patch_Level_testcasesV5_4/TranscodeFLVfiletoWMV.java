...
* 																	>Upload a .flv streaming media file<br>  
...
public class TranscodeFLVfiletoWMV extends TesterAsserter {
	...
	public void test_TranscodeFLVfiletoWMV() throws Exception 
		{
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			String str_path ="Global_variables.xml";
			String local_path ="TranscodeFLVfiletoWMV.xml";
			...
			String VAR_FLVfile = utilityFunction.GetValue(local_path ,"VAR_FLV_file");
			//Variable for the wmv fle name
			String VAR_FLV_fileName = utilityFunction.GetValue(local_path ,"VAR_FLVfileName");
			...
			String VAR_TranscodingWMVProfile=  utilityFunction.GetValue(str_path, "VAR_TranscodingWMVProfile");
			...
			//Reusable action for Login to VCC
     		...
     		//click Transcoding Nodes
     		 ...
     		 //wait
     		 selenium.waitForPageToLoad("30000");
     		//verify if trnascoding node exists otherwise create a new one
 				...
 				for (int second = 0;; second++) {
 					...
 					Thread.sleep(1000);
 		              for (int j = 2; j<100; j++){
 		    	    	...
 		    	    	if (str_val.equals(VAR_TranscodingWMVProfile)){
         	...
         	//Reusable action for WMV file upload
     		 ...
     		 selenium.click("link="+VAR_FLV_fileName);
     				 ...
     				 for(int sec = 0;; sec++) {
     					 ...
     					 try { 
     						 if (selenium.isElementPresent(EnvObjectMap_Rep.lst_transcodetextfield)) {
     							 ...
     							 utilityFunction.waitForChangesToReflect();
     							 utilityFunction.waitForChangesToReflect();
     							 selenium.select(EnvObjectMap_Rep.lst_transcodeprofilefield, "label=" +VAR_TranscodingWMVProfile);
     							 selenium.clickAt(EnvObjectMap_Rep.lst_transcodeprofilefield, "label=" +VAR_TranscodingWMVProfile);
     							 utilityFunction.waitForChangesToReflect();
     							 utilityFunction.waitForChangesToReflect();
     							 selenium.click(EnvObjectMap_Rep.btn_starttranscoding);
     							 utilityFunction.waitForChangesToReflect();
     							 utilityFunction.waitForChangesToReflect();
     							 assertTrue("TranscodeFLVfiletoWMV","Verify that file has transcoded successfully",date,selenium.isTextPresent(VAR_FLV_fileName));
     		...
     		for (int secs = 0;; secs++) {
				 ...
				 try { 
					 if (selenium.isTextPresent(VAR_FLV_fileName +","+" "+"transcoded"))
					 {
						 assertTrue("TranscodeFLVfiletoWMV","Ensure that Transcoded file is displayed on Content page.",date,selenium.isTextPresent(VAR_FLV_fileName +","+" "+"transcoded"));	
			...
			//select the transcoded file
     		 ...
     		 if (str_var.contains(VAR_FLV_fileName +","+" "+"transcoded")){
     			 ...
     			 for (int i = 3; i<100; i++){
     				 ...
     				 if (str_var2.contains(VAR_FLV_fileName +","+" "+"transcoded")){
     			 ...
     			 for (int secs = 0;; secs++) {
     				 ...
     				 try { 
     					 ...
     					 {
     						 assertTrue("TranscodeFLVfiletoWMV","Ensure that ingest is completed for the transcoded file.",date,selenium.isTextPresent("INGESTCOMPLETE"));
     					 }
     					 break; } catch (Exception e) {}
     					 Thread.sleep(1000);
     			 ...
     			 //Variable for VOD program name
     			 String VAR_TranscodedFLV_fileName= VAR_FLV_fileName+", "+"transcoded";
     			 //Reusable action for Create VOD with transcoded WMV file
     			 Create_Program.test_Create_vodProgram(selenium, str_ProgName, VAR_TranscodedFLV_fileName);
     			 //Wait
     			 utilityFunction.waitForChangesToReflect();
     			 ...
     			 assertTrue("TranscodeFLVfiletoWMV","Verify that asx file is generated successfully for a WMV file transcoded into a flv file.",date,utilityFunction.assertTextContains(htmlsource, VAR_FLV_fileName+","+" "+"transcoded"));
     			 assertTrue("TranscodeFLVfiletoWMV","Verify that wmv file is displayed in the asx file for a flv file transcoded into a wmv file.",date,utilityFunction.assertTextContains(htmlsource, "barsandtone.wmv"));
     			 selenium.stop();

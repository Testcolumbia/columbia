...
public class SVP_VOD_WMA_SL_disabled extends TesterAsserter {
	...
	public void test_SVP_VOD_WMA_SL_disabled() throws Exception {
		...
		if (selenium.isElementPresent(EnvObjectMap_Rep.drp_selectTimeZoneCVP))
			...
			assertTrue("SVP_VOD_WMA_SL_disabled","Verify that http protocol is displayed in asx file for a SVP containing .wma file with HTTP delivery option and SL disabled.",date,utilityFunction.assertTextContains(htmlsource, "http://" + VAR_AP_Host + ":80/" + VAR_AP_MountPoint));
			utilityFunction=null;
		}catch(Exception e){
			e.printStackTrace();}
			 finally{

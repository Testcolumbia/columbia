...
public class TranscodedfileIngestVerification extends TesterAsserter {
	...
	public void test_TranscodedfileIngestVerification() throws Exception 
		{
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			String str_path ="Global_variables.xml";
			String local_path ="TranscodedfileIngestVerification.xml";
    		...
    		//verify if transcoding node exists otherwise create a new one
 		    	...
 		    	selenium.click(EnvObjectMap_Rep.btn_transcodeloadprofilebtn);
 				  ...
 				  for (int second = 0;; second++) {
 					...
 					Thread.sleep(1000);
 		    	    	...
 		    	    	if (str_val.equals(VAR_TranscodingProfile)){
     				 ...
     				 for(int sec = 0;; sec++) {
     					 ...
     					 try { 
     						 if (selenium.isElementPresent(EnvObjectMap_Rep.lst_transcodetextfield)) {
     							 ...
     							 utilityFunction.waitForChangesToReflect();
     						    	utilityFunction.waitForChangesToReflect(); 
     							 assertTrue("TranscodedfileIngestVerification","Verify that file has transcoded successfully",date,selenium.isTextPresent(VAR_WMV_fileName+","+" "+"transcoded"));
     		...
     		for (int secs = 0;; secs++) {
				 ...
				 try { 
					 ...
					 {
						 assertTrue("TranscodedfileIngestVerification","Ensure that Transcoded file is displayed on Content page.",date,selenium.isTextPresent(VAR_WMV_fileName +","+" "+"transcoded"));	
     		 ...
     		 if (str_var.contains(VAR_WMV_fileName +","+" "+"transcoded")){
     			 ...
     			 for (int secs = 0;; secs++) {
     				 ...
     				 try { 
     					 ...
     					 {
     						 assertTrue("TranscodedfileIngestVerification","Ensure that ingest is completed for the transcoded file.",date,selenium.isTextPresent("INGESTCOMPLETE"));
     					 }
     					 break; } catch (Exception e) {}
     					 Thread.sleep(1000);
     			 ...
     			 assertTrue("TranscodedfileIngestVerification","Verify that asx file is generated successfully for a WMV file transcoded into a flv file.",date,utilityFunction.assertTextContains(htmlsource, VAR_WMV_fileName+","+" "+"transcoded"));
     			 assertTrue("TranscodedfileIngestVerification","Verify that wmv file is displayed in the asx file for a WMV file transcoded into a flv file.",date,utilityFunction.assertTextContains(htmlsource, VAR_WMVfile));
     			 selenium.stop(); 

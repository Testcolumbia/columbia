...
public class SVP_Content_upload_WMV extends TesterAsserter {
	...
	public void test_SVP_Content_upload_WMV() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			...
			utilityFunction.waitForChangesToReflect();
			utilityFunction.waitForChangesToReflect();
			if(!selenium.isTextPresent(VAR_WMVfilename))
			{
			...
			assertTrue("SVP_Content_upload_WMV","Social VP: Ensure that WMV file is uploaded in Content page.",date,selenium.isTextPresent(VAR_WMVfilename));
			utilityFunction=null;
		}catch(Exception e){
			e.printStackTrace();}
			 finally{

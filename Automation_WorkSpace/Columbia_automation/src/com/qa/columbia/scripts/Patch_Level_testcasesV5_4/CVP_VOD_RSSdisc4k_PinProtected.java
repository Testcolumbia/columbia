...
public class CVP_VOD_RSSdisc4k_PinProtected extends TesterAsserter {
	...
	public void test_CVP_VOD_RSSdisc4k_PinProtected() throws Exception {
		...
		//	utilityFunction.waitForChangesToReflect();
			...
			for (int i =4;i<=10;i++)
			{
				...
				if(selenium.getText(EnvObjectMap_Rep.lbl_pinStatusVODSummaryPage).equals("Yes"))
				{
					...
					assertTrue("CVP_VOD_RSSdisc4k_PinProtected","Verify that Pin protected On-demand program is created for the Pin Protected call.",date,pinStatus);
				}
				break;
				}
			}
		...
		assertTrue("CVP_VOD_RSSdisc4k_PinProtected","Verify that Pin is asked on playing the discovered pin protected On-demand program on viewer portal.",date,selenium.isElementPresent(EnvObjectMap_Rep.txt_privateCode));
		...
		//Verify the asx file
		assertTrue("CVP_VOD_RSSdisc4k_PinProtected","Verify that asx file is generated successfully for the discovered pin protected On-demand program.",date,utilityFunction.assertTextContains(htmlsource2, Var_VODProgramName + "hi.asf"));
		assertTrue("CVP_VOD_RSSdisc4k_PinProtected","Verify that http protocol is displayed in asx file for the discovered pin protected On-demand program.",date,utilityFunction.assertTextContains(htmlsource2, "http"));

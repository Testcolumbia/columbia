/*
* Classname                                                        SVP_Sm_vpv_012_NonPin_VOD_RSSdisc2k 
*
* Version info
*
* Copyright notice
*
* Test Case reference:												Social VP: Ensure that a non pin protected on-demand program is created through RSS 4000 Version4 polycom discovery.
...
* 																	>Configure the RSS4kV6 device and map the above created template programs to it<br>
* 																	>Place a call from the Endpoint to the RSS device.<br>
* * 																>Hang the call from the Endpoint to the RSS device.<br>
* 																	>Notice that the On-demand program is created for the call<br> 	
*  													     			>Play the program on the viewer portal<br> 
*   																>Verify the stream in the .asx file<br> 																			 
* What Verification it performs:	 								1.Verify that Pin-protected on-demand program is created for the Non-pin protected call using RSS2kV4 discovery.<br> 
*																															
...
public class SVP_Sm_vpv_012_NonPin_VOD_RSSdisc2k extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_012_NonPin_VOD_RSSdisc2k() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			...
			utilityFunction.waitForElementToVisible(selenium, EnvObjectMap_Rep.BTN_More_Search, 300);
			...
			int archivedMedia_count = PlaceCall_endpoint.getCount_ArchiveRSS(selenium, VAR_RSSDeviceHostIP);
			...
			selenium.click(EnvObjectMap_Rep.LNK_scheduleProcesses);
			selenium.waitForPageToLoad("30000");
			selenium.click(EnvObjectMap_Rep.LNK_runNowarchiveDiscovery2k);
			selenium.waitForPageToLoad("30000");
			utilityFunction.waitForChangesToReflect();
			selenium.click(EnvObjectMap_Rep.LNK_runNowarchiveDiscovery2k);
			selenium.waitForPageToLoad("30000");
			...
			for (int i =4;i<=10;i++)
			{
				if(selenium.getText(MessageFormat.format(EnvObjectMap_Rep.lnk_DiscoveredProgramRSS2k, i)).startsWith(VAR_ProgPrefix))
					...
					assertTrue("Sm_vpv_012","Verify that automated file broadcast program is created for the live call.",date,selenium.isTextPresent(Var_LiveProgramName));
					break;
					}			
			}						
			utilityFunction.waitForChangesToReflect();
			...
			assertTrue("Sm_vpv_012","Verify that http protocol is displayed in asx file.",date,utilityFunction.assertTextContains(htmlsource, "http"));
			utilityFunction.waitForChangesToReflect();
			utilityFunction=null;
			}catch(Exception e)
			{

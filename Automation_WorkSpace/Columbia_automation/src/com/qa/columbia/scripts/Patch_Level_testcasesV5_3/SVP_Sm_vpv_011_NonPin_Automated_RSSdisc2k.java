/*
* Classname                                                        SVP_Sm_vpv_011_NonPin_Automated_RSSdisc2k 
*
* Version info
*
* Copyright notice
*
* Test Case reference:												Social VP: Ensure that a non pin protected automated program is created through RSS 2000 Version4 polycom discovery.
...
* 																	>Configure the RSS2kV4 device and map the above created template programs it<br>
...
public class SVP_Sm_vpv_011_NonPin_Automated_RSSdisc2k extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_011_NonPin_Automated_RSSdisc2k() throws Exception {
		{
			try{
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
			...
			for (int i =4;i<=10;i++)
			{
				if(selenium.getText(MessageFormat.format(EnvObjectMap_Rep.lnk_DiscoveredProgramRSS2k, i)).startsWith(VAR_ProgPrefix))
					...
					assertTrue("Sm_vpv_011","Verify that automated file broadcast program is created for the live call.",date,selenium.isTextPresent(Var_LiveProgramName));
					break;
					}			
			}		
			//open viewer portal 
		...
		//	assertTrue("SVP_Automated_RSSdisc2k_NonPinProtected","Verify that asx file is generated successfully.",utilityFunction.assertTextContains(htmlsource, VAR_WMEProfileName));
		    assertTrue("Sm_vpv_011","Verify that mms protocol is displayed in asx file.",date,utilityFunction.assertTextContains(htmlsource, "mms"));
			utilityFunction.waitForChangesToReflect();

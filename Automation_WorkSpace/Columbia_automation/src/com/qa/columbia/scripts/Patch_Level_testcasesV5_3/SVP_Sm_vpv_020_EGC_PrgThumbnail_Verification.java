/*
* Classname                                                        SVP_Sm_vpv_020_EGC_PrgThumbnail_Verification 
*
* Version info
*
* Copyright notice
...
public class SVP_Sm_vpv_020_EGC_PrgThumbnail_Verification extends TesterAsserter {
	...
	public void test_SVP_Sm_vpv_020_EGC_PrgThumbnail_Verification() throws Exception {
		{
			try {
			setUp();
			Utility_Functions utilityFunction = new Utility_Functions();
 		  ...
 		  String SVP_url = Var_Columbia_vpURL + VAR_DomSocial + "/home.vp"; 
         	...
         	utilityFunction.waitForChangesToReflect(); 
            utilityFunction.waitForChangesToReflect();
    		//Reusable action to login to Social Viewer portal as EGC user 
          	Login.LoginSVP(selenium, VAR_userEmailID, VAR_userPassword);  
         	utilityFunction.waitForChangesToReflect(); 
            utilityFunction.waitForChangesToReflect();
        	...
        	for (int i = 1; i<=6; i++){
 			...
 			if(i==6)
 			{
 			assertTrue("Sm_vpv_020","Social VP: Ensure that thumbnails(6) are extracted for EGC content.",date,selenium.isElementPresent("selectedThumbnail" +i));
 		 	}
 			else{
